...
const TransitUpload = Transit => {
  ...
  const registerVehicles = async(e) => {
    ...
    if(check){
        await axios.post(process.env.REACT_APP_API_URL+'/transitVehicleRegistration', {vehicleData: excelData, transitId: quarryDetails.transitId}).then(res => {
  ...
  return (
        isLoading ? <Loader /> : (
            ...
            <h1 className='text-success'>Quarry - <span className='text-primary bg-dark'>{quarryDetails?.register_name}</span></h1>
