...
app.post('/mine_login', (req, res) => {
    const {username, password} = req.body;

    let query = `select userId, username, role, mine_id, (SELECT mineral_name FROM mine_details WHERE mine_id=mine_credentials.mine_id) AS mineName from mine_credentials where username=? and password=?`;
    let values = [username, password]
...
app.post('/mines_list', (req, res) => {
    ...
    let query = `SELECT mine_id, lessee_name, village FROM mine_details`;
    let values = [userId]
...
app.post('/mineVehicleRegistration', (req, res) => {
    const { vehicleData, mine_id }  = req.body;
    let sqlstatement = '';
    let TABLE_NAME = 'permit_details_mine';
    ...
    vehicleData.map(async(x) => {
        ...
        keylist += ", mine_id, orderStatus, dateOfBooking)";
        valuelist += `, ${mine_id}, 0, NOW())`;
        sqlstatement = "INSERT INTO " + TABLE_NAME + " " + keylist + " VALUES " + valuelist + ";"
        await db.query(sqlstatement);
        console.log(sqlstatement)
        console.log("success")
...
app.post('/vehicle_check', (req, res) => {

    const { vehicleNumber, mine_id, userId } = req.body;
    let query = `SELECT * FROM permit_details_mine as pm, (select * from mine_details as md where md.mine_id=?) as details WHERE vehicle_no=? and orderStatus=0 and pm.mine_id=? order by dateOfBooking desc`;
    let values = [mine_id, vehicleNumber, mine_id]
...
app.get('/get_signatures/:mine_id', (req, res) => {
    const { mine_id } = req.body;

    res.sendFile(path.join(__dirname, `public/images/signatures/sign_${req.params['mine_id']}.png`), (err) => {
        if (err) {
            console.error(err);
...
app.post('/dispatch', (req, res) => {
    const { dispatch_slip_no, serial_no, travelling_date, required_time, quantity, vehicleNumber, mine_id, destination_address, via_route, driver_license_no, driver_name, driver_ph_no, permit_id } = req.body;
    let query = `UPDATE permit_details_mine SET orderStatus=1, dispatch_slip_no=?, serial_no=?, travelling_date=?, required_time=?, quantity=?, destination_address=?, via_route=?, driver_license_no=?, driver_name=?, driver_ph_no=? WHERE mine_id=? and vehicle_no=? and permit_id=? limit 1`;
    let values = [dispatch_slip_no, serial_no, travelling_date, required_time, quantity, destination_address, via_route, driver_license_no, driver_name, driver_ph_no, mine_id, vehicleNumber, permit_id]
